...
void calcDepthOptimized(float *depth, float *left, float *right, int imageWidth, int imageHeight, int featureWidth, int featureHeight, int maximumDisplacement)
{
    ...
    #pragma omp parallel shared(imageWidth,imageHeight,maximumDisplacement,left,right)
    {
      #pragma omp for
     for (int y = featureHeight; y < y_bound; y++)
     {
            for (int x = featureWidth; x < x_bound; x++)
            {
                ...
                //#pragma omp parallel shared(x,y,imageWidth,imageHeight,maximumDisplacement,left,right)
                //{
                    for (int dx = -maximumDisplacement; dx <= maximumDisplacement; dx++)
                    {
                        for (int dy = -maximumDisplacement; dy <= maximumDisplacement; dy++)
                        {
                            ...
                            for (int boxY = -featureHeight; boxY <= featureHeight; boxY++)
                            {
                                ...
                                if(tail_case == 1)
                                {
                                    difference = left[leftY * imageWidth + leftX] - right[rightY * imageWidth + rightX];
