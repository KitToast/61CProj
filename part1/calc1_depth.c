...
static unsigned char *check_bounds(unsigned char *, int, int, int, int, int, int); //Checks if a feature's pixels will be in bounds
static int check_within_image(int, int, int, int, int, int);
...
void calc_depth(unsigned char *depth_map, unsigned char *left,
    ...
    for(int i  = 0; i < image_height; i++) {
    	...
    	for(int j = 0; j < image_width; j++) {
            ...
            left_feature_patch = check_bounds(left, width_offset, feature_patch_width, feature_patch_height, image_width, image_height, i); //Get feature patch for pixel at position width_offset
...
unsigned char scan_right_image(unsigned char *image,
	...
	int current_offset, appropriate_height; //Will contain the offset
    
	int left_pixel_y = pixel_offset / image_width;
	int left_pixel_x = pixel_offset % image_width;
	...
	int corner_height = pixel_offset / image_width - (search_field_height / 2);
	...
	for(int i = 0; i < search_field_height; i++) {
	  ...
	  current_offset = search_area_offset + image_width * i; //offset which will represent the corner of the feature to be made in the iteratior.
	  appropriate_height = (corner_height + i); //height in which

	  for(int j = 0; j < search_field_width; j++, current_offset++) {
	      
	   int within_image = check_within_image(current_offset, feature_patch_width, feature_patch_height, image_width, image_height, appropriate_height); //Add cases here. Pl0x
	   feature_to_examine = (within_image) ? populate_feature_patch(feature_patch_height, feature_patch_width, current_offset, image) : NULL;
	   	...
	   	if(feature_to_examine) { //If not null
			if((distance_to_examine = squared_euclidean_distance(feature_to_examine,
			    ...
			    current_x = current_offset % image_width;
...
unsigned char *check_bounds(unsigned char *image,
                            ...
                            int image_width,
                            int image_height,
			    int app_height) { //App height is the height of the pixel in question of the left image
	...
	int within_image = check_within_image(corner_offset, feature_patch_width, feature_patch_height, image_width, image_height, app_height - (feature_patch_height / 2)); 
 ...
 */
int check_within_image(int starting_point_offset, int feature_patch_width, int feature_patch_height, int image_width, int image_height, int appropriate_height) {
	...
	//Ends
	if((starting_point_offset / image_width) != appropriate_height) {
	  return 0;
	}
